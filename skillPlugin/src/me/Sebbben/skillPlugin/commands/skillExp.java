    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] ars) {
        if(sender instanceof Player) {
            if(cmd.getName().equalsIgnoreCase("skillexp")){
                if(main.playerData.get(((Player) sender).getUniqueId().toString()) != null) {
                    sender.sendMessage(String.valueOf(main.playerData.get(((Player) sender).getUniqueId().toString()).getExp()));
                } else {
